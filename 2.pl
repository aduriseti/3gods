...
% query/3 now delegates the evaluation to the helper predicate.
...
query_position(Position, Question, WorldState) :- 
        (   at_position(Position, truly, WorldState), query(truly, Question, WorldState)).
query_position(Position, Question, WorldState) :- 
        (   at_position(Position, falsely, WorldState), query(falsely, Question, WorldState)).
