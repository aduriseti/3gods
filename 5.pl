...
% A "family" is now defined as either the world where all gods are 'truly'
% or the world where all gods are 'falsely'.
% produces
% [
%   pos(1, truly, _),
%   pos(2, truly, ),
...
% --- Tests for Core Logic (`answer_for_world`) ---

test('trivial question "true" is true in a truly world') :-
    build_uniform_family(truly, F),
    generate_worlds_from_templates(F, World),
    answer_for_world(true, 1, World, true).

test('trivial question "true" is false in a falsely world') :-
    build_uniform_family(falsely, F),
    generate_worlds_from_templates(F, World),
    answer_for_world(true, 1, World, fail).

test('at_pos(1,truly) is true in a truly world') :-
    build_uniform_family(truly, F),
    generate_worlds_from_templates(F, World),
    answer_for_world(at_position_question(1, truly), 1, World, true).

test('at_pos(1,truly) is ALSO true in a falsely world (due to lying)') :-
    build_uniform_family(falsely, F),
    generate_worlds_from_templates(F, World),
    answer_for_world(at_position_question(1, truly), 1, World, true).

% --- Tests for Family Signatures ---

test('signature for "all truly" family with question "true" is true') :-
    build_uniform_family(truly, FamilyTemplate),
    family_signature(true, 1, FamilyTemplate, true).

test('signature for "all falsely" family with question "true" is fail') :-
    build_uniform_family(falsely, FamilyTemplate),
    family_signature(true, 1, FamilyTemplate, fail).

test('at_pos(1,truly) is NOT a distinguishing question (signatures collide)') :-
    build_uniform_family(truly, F_True),
    build_uniform_family(falsely, F_False),
    family_signature(at_position_question(1, truly), 1, F_True, Signature),
    family_signature(at_position_question(1, truly), 1, F_False, Signature).

test('check 1: there should be no internal inconsistency for question "true"', [fail]) :-
    % This test asserts: "There is a family that is NOT internally consistent for this question."
    % This test SHOULD fail, because both of our families are consistent.
    % If it SUCCEEDS, it means family_signature/4 is broken.
    generate_world_family_template(Family),
    \+ family_signature(true, 1, Family, _).

test('check 2: there should be no signature collision for question "true"', [fail]) :-
    % This test asserts: "There are two different families that have the SAME signature."
    % This test SHOULD fail, because their signatures should be different (true and fail).
    % If it SUCCEEDS, it means the core evaluation logic is giving the wrong answer for one family.
    generate_world_family_template(Family1),
    generate_world_family_template(Family2),
    Family1 \= Family2,
    family_signature(true, 1, Family1, Signature),
    family_signature(true, 1, Family2, Signature).

% --- Final Integration Test ---

test('trivial question "true" IS a distinguishing question') :-
    is_distinguishing_question(true, 1).
