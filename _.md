...
World constraints are very simple - we have a single position, and it may be inhabited by either the Truly or Falsely god. 
...
Note that if we are allowed multiple questions (3 for this puzzle), we don't just have to find $N$ questions - we have to find $2^N-1$ questions to build up a tree. Each question will be a node, and answers to questions ("ja"/"da") will be edges.
...
graph TD
    ...
    Left -->|Q2: da| L_Da["{TRF, RTF}"]
    Left -->|Q2: ja| L_Ja["{FRT, RFT}"]

    Right -->|Q3: da| R_Da["{FTR, RTF}"]
    Right -->|Q3: ja| R_Ja["{TFR, RFT}"]
...
I'm still a little surprised Prolog can't evaluate a million questions in 10 seconds, but - oh well - can't argue with a hanging terminal.
...
For example, let's say we pick "Is 1==1?" as our starting question and ask it to the god in the first position. What are the possible responses for each permutation of the gods?
- TFR: either "ja" or "da" depending on the meaning of the language of the gods
- TRF: ditto
- FTR: ditto
- FRT: ditto
